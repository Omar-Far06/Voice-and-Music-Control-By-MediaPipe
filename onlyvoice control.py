...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks and result.multi_handedness:
        for hand_landmarks, hand_label in zip(result.multi_hand_landmarks, result.multi_handedness):
            ...
            if label == "Left":  # استخدام اليد اليمنى فقط
                ...
                if is_hand_open(landmarks):  # إذا كانت اليد مفتوحة
                    ...
                    cv2.putText(frame, "Right Hand Open: Volume Up", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                else:  # إذا كانت اليد مغلقة
                    current_volume = volume.GetMasterVolumeLevel()
                    new_volume = max(current_volume - 0.5, min_vol)  # خفض الصوت
                    volume.SetMasterVolumeLevel(new_volume, None)
                    cv2.putText(frame, "Right Hand Closed: Volume Down", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    ...
    if key == ord('q') or key == ord('z'):  # إضافة خيار "z" للخروج
        break
